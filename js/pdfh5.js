  ...
  function (g, pdfjsWorker, pdfjsLib, version) {
    ...
    var definePinchZoom = function () {
      var PinchZoom = function (el, options, pinchParentNode) {
          ...
          this.options.tapZoomFactor = isNaN(options.tapZoomFactor)
            ? 2
            : options.tapZoomFactor
          this.options.zoomOutFactor = isNaN(options.zoomOutFactor)
            ? 1.2
            : options.zoomOutFactor
          this.options.animationDuration = isNaN(options.animationDuration)
            ? 300
          ...
          this.options.dampingFactor = isNaN(options.dampingFactor)
            ? 0.85
      ...
      PinchZoom.prototype = {
          ...
          var velocityX =
            validRecords.length > 1
              ? (validRecords[validRecords.length - 1].x - validRecords[0].x) /
                (validRecords[validRecords.length - 1].time -
                  validRecords[0].time)
              : 0
        ...
        startInertiaAnimation: function (initialVelocity) {
          var minVelocity = 0.08 // 鏈€灏忓仠姝㈤€熷害
          var decay = 0.92 // 閫熷害琛板噺绯绘暟
          ...
          var animateFrame = () => {
            ...
            // 搴旂敤閫熷害
            this.addOffset({
              x: initialVelocity * 16, // 16ms 姣忓抚鐨勮ˉ鍋垮€�
              y: 0
            })
        ...
        scale: function (scale, center) {
          ...
          this.done &&
            this.done.call(this, this.getInitialZoomFactor() * this.zoomFactor)
        },
        ...
        getCurrentZoomCenter: function () {
          var length = this.pinchContainer.offsetWidth * this.zoomFactor,
            ...
            centerX =
              (widthOffsetRatio * this.pinchContainer.offsetWidth) /
              (widthOffsetRatio + 1),
            height = this.pinchContainer.offsetHeight * this.zoomFactor,
            offsetTop = this.offset.y,
            offsetBottom =
              height - offsetTop - this.pinchContainer.offsetHeight,
            heightOffsetRatio = offsetTop / offsetBottom,
            centerY =
              (heightOffsetRatio * this.pinchContainer.offsetHeight) /
              (heightOffsetRatio + 1)
        ...
        setupMarkup: function () {
          this.pinchContainer = buildElement(
        ...
        bindEvents: function () {
          ...
          Array.from(this.el.querySelectorAll('canvas')).forEach(function (
            imgEl
          ) {
      ...
      var detectGestures = function (el, target) {
        var interaction = null,
          ...
          detectDoubleTap = function (event) {
            ...
            if (
              time - lastTouchStart < 300 &&
              Math.abs(pageY - lastTouchY) < 10 &&
              Math.abs(lastTop - top) < 10
            ) {
              cancelEvent(event)
    ...
    Pdfh5.prototype = {
      init: function (options) {
        ...
        this.options.zoomEnable =
          this.options.zoomEnable === false ? false : true
        this.options.scrollEnable =
          this.options.scrollEnable === false ? false : true
        this.options.loadingBar =
          this.options.loadingBar === false ? false : true
        ...
        this.options.fullscreen =
          this.options.fullscreen === false ? false : true
        this.options.renderType =
          this.options.renderType === 'svg' ? 'svg' : 'canvas'
        ...
        if (
          this.options.logo &&
          Object.prototype.toString.call(this.options.logo) ===
            '[object Object]' &&
        ...
        if (
          !(
            ...
            (this.options.background.color || this.options.background.image)
          )
        ) {
        ...
        viewerContainer.addEventListener('scroll', function () {
          ...
          if (
            scrollTop + self.container.offsetHeight >=
        ...
        this.fn1 = function () {
          ...
          self.viewer.style.transform =
            'scale(' + s1 + ', ' + s2 + ') translate(' + x + 'px, 0px)'
          ...
          if (self.options.type === 'ajax') {
            ...
            xhr.onerror = function (err) {
              ...
              if (arr1 && arr1 instanceof Array) {
                for (var i = 0; i < arr1.length; i++) {
                  arr1[i] &&
                    arr1[i].call(self, 'error', err.statusText, self.endTime)
                }
              }
            ...
            for (var i = 0; i < arr1.length; i++) {
              arr1[i] &&
                arr1[i].call(
                  self,
                  'error',
                  'Expect options.pdfurl or options.data!',
                  self.endTime
                )
            }
          }
              ...
              promise = promise.then(
                function (pageNum) {
                  return self.thePDF.getPage(pageNum).then(function (page) {
                    ...
                    var div = self.container.querySelector(
                      '.pageContainer' + pageNum
                    )
                    var container
                    if (!div) {
                      ...
                      if (window.ActiveXObject || 'ActiveXObject' in window) {
                        ...
                        container.style.width = viewport.width + 'px'
                        container['data-scale'] =
                          viewport.width / viewport.height
                      } else {
                        var h =
                          container.offsetWidth /
                          (viewport.viewBox[2] / viewport.viewBox[3])
                    ...
                    if (options.background) {
                      ...
                      if (options.background.repeat) {
                        container.style['background-repeat'] =
                          options.background.repeat
                      }
                      /*鑳屾櫙鍥剧墖浣嶇疆*/
                      if (options.background.position) {
                        container.style['background-position'] =
                          options.background.position
                      }
                      /*鑳屾櫙鍥惧儚鐨勫昂瀵�*/
                      if (options.background.size) {
                        container.style['background-size'] =
                    ...
                    var sum = 0,
                      containerH = self.container.offsetHeight
                    self.pages =
                      self.viewerContainer.querySelector('.pageContainer')
                    if (options.resize) {
          ...
          .then(function (opList) {
            ...
            return svgGfx.getSVG(opList, scaledViewport).then(function (svg) {
              ...
              if (arr1 && arr1 instanceof Array) {
                for (var i = 0; i < arr1.length; i++) {
                  arr1[i] &&
                    arr1[i].call(self, pageNum, time - self.initTime, container)
                }
              }
      ...
      finalRender: function (options) {
        ...
        if (arr1 && arr1 instanceof Array) {
          for (var i = 0; i < arr1.length; i++) {
            arr1[i] &&
              arr1[i].call(self, 'success', 'pdf鍔犺浇瀹屾垚', self.endTime)
          }
        }
